...
object LucraMapper {
  ...
  fun playerStatToMap(stat: LucraPlayerStat?): WritableMap? {
    ...
    map.putString("value", stat.value)
    return map
  }
  ...
  fun scheduleToMap(schedule: LucraSchedule?): WritableMap? {
    ...
    schedule.projectionsPending?.let { map.putBoolean("projectionsPending", it) } ?: map.putNull("projectionsPending")
    return map
  }
  ...
  fun userToMap(user: LucraUser): WritableMap {
    ...
    map.putInt("loyaltyPoints", user.loyalty_points)
    return map
  }
