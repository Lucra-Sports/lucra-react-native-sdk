...
If your app also uses Sentry (for example `@sentry/react-native`), align it to `sentry-cocoa` 8.58.4 or newer within 8.x and the same dynamic product. An older pin or 9.x fails dependency resolution with `could not find compatible versions for pod "Sentry"`.
...
The Lucra Android SDK depends on `io.sentry:sentry` 8.54.0 for its own internal diagnostics. It never calls `Sentry.init` and sends nothing to your Sentry project. If your app also uses Sentry, every `io.sentry:*` artifact must resolve to 8.54.0 or newer; an older version can crash at launch with `Sentry SDK has detected a mix of versions` or fail with `AbstractMethodError`.
