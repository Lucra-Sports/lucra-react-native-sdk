...
Unpack your incoming link and hand it to Lucra for matching. `handleLucraLink`
presents Lucra's full-screen flow when the link matches; headless integrations
that render their own screens should use
[`parseLucraLink`](1.2.3_push_notifications.md#headless-deeplink-parsing)
instead, which resolves the link to `{ flow, matchupId, ... }` without
presenting UI. Its `flow` can always be handed back to
[`LucraSDK.present`](1.2.7_lucraflows.md#presenting-a-parsed-deeplink) if you
decide to show Lucra's own screen after all.
