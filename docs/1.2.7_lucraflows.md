...
- `HANDSHAKE_TOS`: Capture Terms of Service acceptance and complete a handshake sign-in. No params.
  **Present this only in response to a `tosNotAccepted` handshake failure — it is not an entry
  point.** It needs no session, and on agreement the SDK resubmits the sign-in itself. See
  [Handshake Authentication](1.2.12_handshake_authentication.md#terms-of-service).
