...
* Documented that the Activity hosting Lucra flows must leave `android:taskAffinity` unset. See [Getting Started](1.0.0_project_setup.md#task-affinity).
* Added handshake authentication — sign a user into Lucra from a token your own backend signs, with no Lucra login UI. `LucraSDK.registerHandshakeAuthTokenProvider(provider, { bypassTosAgreement? })` stores the provider, `LucraSDK.signInWithHandshakeAuth(): Promise<LucraUser | null>` performs the sign-in, and `LucraSDK.addHandshakeAuthListener({ onError, onInFlightChange })` returns one unsubscribe for both failures and in-flight state. Rejections are typed as the new exported `LucraHandshakeAuthError`; every code except `tosNotAccepted` leaves phone auth as a working fallback. Handshake auth needs Lucra-side tenant provisioning, so talk to Lucra before integrating. See [Handshake Authentication](1.2.12_handshake_authentication.md).
* Added `LucraSDK.FLOW.HANDSHAKE_TOS` — captures Terms of Service acceptance for a user new to Lucra and completes the handshake sign-in itself. Present it only in response to a `tosNotAccepted` rejection; it is not an entry point.
* **Fix:** `LucraSDK.present` now accepts every flow the deeplink parsers can return. `tournamentDetails`, `notifications`, `transactionHistory`, `customerSupport` and `responsibleGaming` come back from `parseLucraLink` and `handleLucraNotification`, but were missing from `LucraSDK.FLOW` — so the documented parse-then-present round trip rejected on Android and crashed the app on iOS. All five are now presentable (`TOURNAMENT_DETAILS` takes a `matchupId`), and an unrecognized flow name rejects the promise on both platforms instead of trapping on iOS.
* Added `theme.themeMode` (`'light' | 'dark' | 'auto'`) to force the appearance every Lucra screen renders in, rather than having it inferred from which palettes you supplied. Omitting it keeps 5.10.0's behavior exactly. Exported the `LucraThemeMode` type. See [Theming and Appearance](1.2.1_theming.md#theme-mode).
