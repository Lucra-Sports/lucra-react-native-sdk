...
* Added `includePrivateViewable` to `LucraSDK.getRecomendedTournaments(…)`, which also returns private tournaments that are viewable without a join code — a code is still required to join, so check the new `isPrivate` field. Also added `totalParticipants`, which is the number to display: `participants` can hold only a subset of entrants on large tournaments.
  * **iOS only.** The Android SDK exposes the flag and both fields only on a lighter query whose payload drops `participants[].rewardValue` and `payoutStructure`, so the bridge keeps the current query rather than silently regressing the Android response shape. Android ignores the flag and leaves both fields `undefined`. See [Tournament Headless Functions](3.3_tournaments_headless.md#platform-differences).
