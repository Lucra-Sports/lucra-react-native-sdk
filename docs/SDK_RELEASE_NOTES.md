...
* Bumped Android to [7.0.1](https://github.com/Lucra-Sports/lucra-android-sdk/releases/tag/7.0.1)
* Added **handshake authentication** — signs your user into Lucra from a token your own backend signs, with no Lucra login screen. Phone auth remains the fallback. See [Handshake Authentication](1.2.12_handshake_authentication.md).
  ...
  * `LucraSDK.signInWithHandshakeAuth(): Promise<LucraUser>` resolves once the profile has loaded. An existing session wins, so it is safe to call unconditionally at app start; call `logout()` first to switch users.
  * A provider that rejects surfaces as `providerFailed` carrying **your** error's message, rather than timing out — the bridge correlates each token request by id and propagates the rejection into the native provider.
  * `LucraSDK.subscribeToAuthState(onChange)` (plus `getAuthState()` and a `useAuthState()` hook) reports `isHandshakeAuthInFlight`, the last `handshakeAuthError`, and `isResolvingAuthState`. Because handshake failures fall back to phone auth silently, this is how a working fallback is told apart from a broken integration. All three ride one payload, so the in-flight flag can never be observed dropping before the error that caused it.
  * Added the `HANDSHAKE_TOS` flow. A user new to Lucra fails with `tosNotAccepted` — the one failure that must **not** fall back to phone auth, since phone auth cannot create the account either. Present `LucraSDK.FLOW.HANDSHAKE_TOS`; the SDK captures the agreement and resubmits the sign-in itself.
  * Exported `LucraAuthState`, `LucraHandshakeAuthError` and `LucraHandshakeAuthErrorCode`. `profileTimedOut` is Android-only; `isResolvingAuthState` is iOS-only and is `null` on Android, whose SDK exposes no equivalent.
  * The example app gained a "Handshake Auth" screen with selectable provider behaviours (working / throws / hangs / empty), each driving a different native error code, plus a live auth-state readout and the `tosNotAccepted` → `HANDSHAKE_TOS` path.
* Added `themeMode` to `theme`: `'light' | 'dark' | 'system'` forces the SDK's appearance instead of deriving it from which palettes you supply. Omitting it preserves the previous behaviour exactly, so this is not a breaking change. If a forced mode has no matching palette the SDK reuses the one you did supply, and the wrapper now warns in Metro when that happens — the Android SDK logs it only to Logcat and iOS logs nothing. Exported `LucraThemeMode`. See [Theming and Appearance](1.2.1_theming.md#forcing-light-or-dark).
  * **Android fix:** a theme carrying no color tokens (`{ themeMode: 'light' }`, or one whose colors were all dropped as invalid) built an empty `ColorStyle` anyway, which made the SDK log a "reusing your palette" warning for a palette nobody supplied. The bridge now sends `null` instead.
* Added `includePrivateViewable` to `LucraSDK.getRecomendedTournaments(…)`, which also returns private tournaments that are viewable without a join code — a code is still required to join, so check the new `isPrivate` field. Also added `totalParticipants`, which is the number to display: `participants` can hold only a subset of entrants on large tournaments.
  * Works on both platforms. On Android it requires Lucra Android SDK 7.0.1 or newer, which this release depends on; against an older Android SDK the flag is ignored and both fields are `undefined`. See [Tournament Headless Functions](3.3_tournaments_headless.md#platform-differences).
  * `rewardType` and `payoutStructure` are absent from recommended-list results on both platforms — neither native query returns them. Use `getTournamentDetails` for a tournament's reward structure.
* **iOS fix:** calling any bridge method before `LucraSDK.init()` resolved messaged a nil object, which in Objective-C is a silent no-op — the returned promise never settled. The handshake methods route through a nil-safe accessor and reject with `notInitialized` instead. (The other bridge methods still have this behaviour; it is tracked separately.)
