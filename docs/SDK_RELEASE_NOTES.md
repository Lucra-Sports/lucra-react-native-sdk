...
* Bridged the headless phone-auth flow: `LucraSDK.submitPhoneNumber(phoneNumber)`, `LucraSDK.submitVerificationCode(code): Promise<LucraUser>`, and `LucraSDK.resendCode()`, matching the native iOS/Android SDKs. Phone numbers are normalized per platform (the exported `formatUsPhoneNumber` helper does the normalization), and rejections use the new exported `LucraPhoneAuthError` type. See [Headless Functionality](1.2.9_headless_interactions.md#phone-authentication-headless).
...
* Added `LucraSDK.submitUserScore({ tournamentId, score, isFinal, metadata? }): Promise<PoolTournament | null>` — submits the user's tournament score and resolves with the updated tournament (`null` when iOS returns none). The iOS SDK accepts whole-number scores only, so the score is rounded on iOS. See [Tournament Headless Functions](3.3_tournaments_headless.md#submit-user-score).
* Added `LucraSDK.getTournamentDetails(tournamentId, { leaderboardLimit?, leaderboardOffset? }?): Promise<TournamentDetails>` — the lightweight `ui_tournament_details` payload behind Lucra's in-app tournament details screen (leaderboard, payout structure, attempt data, how-to-play, terms). Prefer it over `tournamentMatchup` for headless tournament UIs; the native SDKs deprecate the heavier call. Leaderboard pagination options are Android-only (iOS returns the first page). Exported the `TournamentDetails` type family. See [Tournament Headless Functions](3.3_tournaments_headless.md#get-tournament-details).
